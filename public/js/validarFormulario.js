...
$(document).ready(function () {
  ...
  $("#idCrearCuenta").on("click", function (event) {
    ...
    $.ajax({
      ...
      data :datosInicio,
      success: function (data, state, jqXHR) { //Si todo ha ido bien pongo un mensaje de acierto
        if (data != null) {
          alert("Cuenta creada con exito")
        } else {
          alert("Datos de cuenta no validos")
        }
      },
...
function validarInicio(datosInicio) { //Toggle es mas bonito que alert
  ...
  return false; // Si la validación pasa, permite el envío del formulario
}
